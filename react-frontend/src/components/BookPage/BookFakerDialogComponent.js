...
import bookFakerFactory from "./bookFakerFactory";
...
const BookFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = bookFakerFactory(fakerCount);
        let results = [];
        setLoading(true);
        try {
            for (let i = 0; i < fakeData.length; i++) {
                const _data = fakeData[i];
                const res = await client.service("book").create(_data);
                // const res = _data;
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(BookFakerDialogComponent);
